...
### Added
- macOS support. The release now ships `Mitel Phone Finder.app` for Apple
  silicon and Intel alongside the Windows executable, built from the same
  source by the same workflow.
- macOS application icon (`assets/appicon.icns`), generated by
  `tools/make_icon.py` next to the Windows `.ico`.

### Changed
- The discovery engine is no longer Windows-only. Interface enumeration uses
  PowerShell on Windows and `ifconfig` / `ip` elsewhere, the default route is
  read with `route -n get default` or `ip route` on POSIX, and the ARP cache is
  parsed from both the Windows and BSD output formats, including BSD's
  abbreviated MAC octets.
...
[1.1.0]: https://github.com/dirazi83/Mitel-6900-SIP-IP-Finder/releases/tag/v1.1.0
